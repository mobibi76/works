...
function bindInterLinkEvent() {
    ...
    // 중복 바인딩 방지
    if (!nav.hasAttribute('data-event-bound')) {
        nav.addEventListener('click', handleInterLinkClick);
        nav.setAttribute('data-event-bound', 'true');
    }
    
    if (!container.hasAttribute('data-event-bound')) {
        container.addEventListener('click', handleInterLinkClick);
        container.setAttribute('data-event-bound', 'true');
    }
}
...
function introFetch(pageTitle) {
    fetch(pageTitle).then(response => {
        ...
        if (containerElement) {
            ...
            bindInterLinkEvent();
            bindPopupCloseEvent();  // 팝업 닫기 리스너 재바인딩
        }
    }).catch(error => {
        const containerElement = document.querySelector('#container');
        containerElement.innerHTML = '<p>Failed to load content. Please try again later.</p>';
...
function openPopup() {
    ...
    if (donotShowAgain !== 'true') {
        ...
        bindPopupCloseEvent();  // 팝업 열릴 때마다 이벤트 리스너 재바인딩
    }
}
...
// Sub3. cancel at external link click
