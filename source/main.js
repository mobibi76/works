...
if (containerElement && containerElement.innerHTML !== text) { // Only update if content has changed
...
if (tooltip.textContent !== text) { // Only update tooltip if content has changed
...
if (document.hidden) {
    stopCanvasAnimation();
} else if (!isAnimating) {  // Only restart if not animating already
